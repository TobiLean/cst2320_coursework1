...
//function handles the signup event
...
//function to check if names (first and last) are valid
...
function checkPassword(x, warningParent) {
  ...
  if (pWordRegExp.test(x.value)) {
    ...
    console.log(`Good password: ${passwordCheck}`);
    warningMsg(
      "Requires: Special character, uppercase, lowercase, at least 8 characters",
      warningParent
    );
  }
}

//function to print warning messages if there's validation error
...
//function to check if user exist already before signing up and logging in
function checkUserExist(typeOfUser = "signup") {
  if (typeOfUser == "login") {
    for (
      let i = 0;
      i < JSON.parse(localStorage.getItem("user_details")).length;
      i++
    ) {
      ...
      if (tempName == uName.value) {
        ...
        if (!sessionStorage.getItem("loggedUser1")) {
          ...
          !sessionStorage.getItem("loggedUser2") &&
          uName.value != sessionStorage.getItem("loggedUser1")
        ) {
    ...
    for (
      ...
      if (
        JSON.parse(localStorage.getItem("user_details"))[i].userEmail !=
        newUser.userEmail
      ) {
        userExist = false;
      } else {
      ...
      if (
        JSON.parse(localStorage.getItem("user_details"))[i].username !=
        newUser.username
      ) {
        userExist = false;
      } else {
...
function storeData() {
  ...
  if (JSON.parse(localStorage.getItem("user_details")) == null) {
    ...
    if (userExist == false) {
      ...
      console.log(newUser.userEmail);
      console.log(
        JSON.parse(localStorage.getItem("user_details"))[0].userEmail
      );
      console.log("--successfully stored user!--");
      alert("--successfully stored user!--");
    } else {
