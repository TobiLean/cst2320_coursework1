...
// variables store the remaining time of users
let player1Time;
let player2Time;

//function to count time down
...
//array containing the order of the board arrangement
...
//assigns the id for cells, mimics chess board cell ids'
function setCellId() {
  var rowNum = 0;
...
//event listeners to determine time mode selected
...
//this handles the start of a game.
function startGame() {
  const kings = Array.from(document.querySelectorAll("#king_piece"));
...
// function handles the ending of a game
function endGame() {
  //array of the kings present on the board.
  const kings = Array.from(document.querySelectorAll("#king_piece"));

  if (
    gameStarted == true &&
    kings.some((king) => king.firstChild.classList.contains("dark_piece")) &&
...
//adding drag event listeners for each cell on the chess board
cells.forEach((cell) => {
...
//prevents the default behavior of the dragover listener
function dragOver(el) {
  el.preventDefault();
}

//handles the drop of a piece on a target cell
...
// this function checks if movement to target cell is valid and return a value if it is
function checkValidMove(target) {
  const targetId =
    target.getAttribute("cell-id") || target.parentNode.getAttribute("cell-id");
  const piece = draggedPiece.id;
  console.log("target cell-id ", targetId);
  console.log("start cell-id ", startPos);
  console.log("piece ", piece);

  //this function checks for obstacles on the path of the piece
  function checkForSpace(
    ...
    //declaring variables to store number of pieces present on the path of a rook and/or bishop
    var piecesPresentBs = 0;
    var piecesPresentRk = 0;

    //if piece is a bishop piece
    if (pieceType == "bs_pc") {

        ////tr-> top right
      if (direction == "tr") {
        if (
          targetId[0].charCodeAt(0) == startPos[0].charCodeAt(0) + i &&
          Number(targetId[1]) == Number(startPos[1]) + i
        ) {
          for (let index = 1; index < Math.abs(i); index++) {
            console.log(
              `[cell-id="${String.fromCharCode(
                targetId[0].charCodeAt(0) - index
              )}` + `${Number(targetId[1]) - index}"]`
            );
            if (
  ...
  //Checking for validity each piece's movement with switch case
  switch (piece) {
    case "pawn_piece":
      const initialRow = [
        "a2",
      ...
      if (
        initialRow.includes(startPos) &&
        startPos[0] == targetId[0] &&
        ((playerTurn == "dark_piece" && Number(targetId[1]) >= 5) ||
      ...
      if (
        !document.querySelector(`[cell-id="${targetId}"]`).firstChild &&
        startPos[0] == targetId[0] &&
      ...
      if (
        document.querySelector(`[cell-id="${targetId}"]`).firstChild &&
        (`${startPos[0]}`.charCodeAt(0) + 1 == `${targetId[0]}`.charCodeAt(0) ||
          `${startPos[0]}`.charCodeAt(0) - 1 == `${targetId[0]}`.charCodeAt(0))
      ) {
        return true;
      }
...
//checks for a win/loss in a game between two logged in players. also calculates number of matches
function checkWin() {
  const kings = Array.from(document.querySelectorAll("#king_piece"));
